...
{
    char __COPY_randNo[16];       /// This array will store copied __random_number value,
    int a=0,n=1,__randNo[8],_rNo;
    __random_number *rg;
...
//cout<<endl;
     for(int i=0,j=8;j<16;j++,i=i+2)
     {
         if(j>11)
         {__COPY_randNo[j]=__COPY_randNo[n];n=n+2;}
         else
         __COPY_randNo[j]=__COPY_randNo[i];
     }
    for(int j=0;j<4;j++)
    {
        for(int k=0;k<4;k++)
